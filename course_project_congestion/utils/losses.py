...
__all__ = ['L1Loss', 'MSELoss', 'DistillationLoss']
...
class DistillationLoss(nn.Module):
    def __init__(self, loss_weight=100.0, alpha=0.8, reduction='mean', sample_wise=False):
        super().__init__()
        self.loss_weight = loss_weight
        self.alpha = alpha
        self.reduction = reduction
    ...
    def forward(self, student_output, teacher_output=None, ground_truth=None, weight=None, **kwargs):
        if teacher_output is None:
            if ground_truth is not None:
                ...
                raise ValueError("Both teacher_output and ground_truth cannot be None")
        else:
